...
#define ETHIF_TX_TIMEOUT                       (20U)
#define ETHIF_TX_MAX_RETRIES                   (2U)
...
/* RAM diagnostics used during bring-up. They have no timing-critical side
 * effects and make each receive/transmit boundary observable over SWD. */
volatile uint32_t ethDiagRxPbuf;
volatile uint32_t ethDiagRxIpv4;
volatile uint32_t ethDiagRxIcmp;
volatile uint32_t ethDiagRxEchoRequest;
volatile uint32_t ethDiagTxPbuf;
volatile uint32_t ethDiagTxIcmp;
static volatile uint32_t ethTotalRxBytes;
static volatile uint32_t ethTotalTxBytes;
static volatile uint8_t ethLinkMbps;
static volatile uint8_t ethFullDuplex;
...
static void low_level_init(struct netif *netif)
{
  ...
  /* lwIP generates protocol checksums in software. This avoids the F7 MAC
     offload path producing invalid ICMP frames while retaining CRC/padding. */
  /* The HAL initializes every Tx descriptor with FULL checksum insertion.
     Include the CSUM attribute even when disabling offload so
     ETH_Prepare_Tx_Descriptors() actually clears those descriptor bits. */
  ...
  RxPktSemaphore = osSemaphoreNew(1U, 0U, NULL);

  /* create a binary semaphore used for informing ethernetif of frame transmission */
  TxPktSemaphore = osSemaphoreNew(1U, 0U, NULL);

  /* create the task that handles the ETH_MAC */
  const osThreadAttr_t ethIfAttributes = {
    .name = "EthIf",
    .stack_size = INTERFACE_THREAD_STACK_SIZE * 4U,
    .priority = osPriorityAboveNormal
  };
  EthIfThread = osThreadNew(ethernetif_input, netif, &ethIfAttributes);
  ...
  { 
    ...
    */
    const osThreadAttr_t rmiiAttributes = {
      .name = "RMII_Watchdog",
      .stack_size = configMINIMAL_STACK_SIZE * 4U,
      .priority = osPriorityNormal
    };
    (void)osThreadNew(RMII_Thread, NULL, &rmiiAttributes);
  }
}
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  if (p != NULL && p->len >= 34U)
  {
    ...
    if (frame[12] == 0x08U && frame[13] == 0x00U && frame[23] == IP_PROTO_ICMP)
    {
      ethDiagTxIcmp++;
    }
  }
  ...
  do
  {
    ...
    {
      ethTotalTxBytes += p->tot_len;
      errval = ERR_OK;
    }
    else
    {
      if((HAL_ETH_GetError(&EthHandle) & HAL_ETH_ERROR_BUSY) &&
         (retryCount < ETHIF_TX_MAX_RETRIES))
      {
        /* Wait for descriptors to become available */
        (void)osSemaphoreAcquire(TxPktSemaphore, ETHIF_TX_TIMEOUT);
    ...
    if (p != NULL)
    {
      ...
      if (p->len >= 34U)
      {
        ...
        if (frame[12] == 0x08U && frame[13] == 0x00U)
        {
          ...
          {
            ethDiagRxIcmp++;
            if (frame[34] == ICMP_ECHO) ethDiagRxEchoRequest++;
          }
        }
      }
    }
...
uint32_t EthernetIf_GetRxBytes(void) { return ethTotalRxBytes; }
uint32_t EthernetIf_GetTxBytes(void) { return ethTotalTxBytes; }
uint8_t EthernetIf_GetLinkMbps(void) { return ethLinkMbps; }
uint8_t EthernetIf_IsFullDuplex(void) { return ethFullDuplex; }
