...
{ 
  ...
  /* Conservative 8 MHz SD clock for reliable streaming from older SDSC cards. */
  uSdHandle.Init.ClockDiv            = 4U;
  ...
  /* Stay in 1-bit mode. At 8 MHz it still exceeds WAV bandwidth by a wide
     margin and is substantially more tolerant of old cards/signal quality. */
...
uint8_t BSP_SD_WriteBlocks(uint32_t *pData, uint32_t WriteAddr, uint32_t NumOfBlocks, uint32_t Timeout)
{
  ...
  for(attempt = 0U; attempt < 3U; attempt++)
  {
    if(HAL_SD_WriteBlocks(&uSdHandle, (uint8_t *)pData, WriteAddr,
